...
在自动机理论驱动的模型检测中，为了验证模型 $M$ 是否满足 $\psi$，我们通常不直接在 Kripke 结构上硬算。相反，我们会：
1.  将 LTL 属性规范 $\psi$（通常是其否定形式 $\neg\psi$）转换为一个等价的 [[LTL 转 Büchi 自动机|Büchi 自动机]]。
2.  将系统模型 $M$ 与该 Büchi 自动机进行同步积（Synchronous Product）。
3.  通过检查乘积自动机的语言是否为空（Emptiness Check）来得出模型是否满足规范的结论。
