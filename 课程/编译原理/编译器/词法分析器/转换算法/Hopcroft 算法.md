## 1. 简介
**最小化 DFA (DFA Minimization)** 是编译原理词法分析流程的最后一步。通过子集构造法生成的 DFA 虽然可以唯一确定输入字符的跳转，但往往会产生许多**冗余或等价**的状态。
为了减少词法分析器在运行时的内存开销和状态表大小，我们需要将 DFA 转换为**状态数最少**的等价 DFA。最常用的算法是 **Hopcroft 算法（基于等价类的划分法）**。
...
## 5. 总结流程
`正则表达式 (RE)` $\xrightarrow{\text{Thompson构造法}}$ `NFA` $\xrightarrow{\text{子集构造法}}$ `DFA` $\xrightarrow{\text{Hopcroft算法}}$ `最小化 DFA` $\rightarrow$ `词法分析器代码`
